...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valueC2H5OH != cur->valueC2H5OH)
    {
        GIZWITS_LOG("valueC2H5OH Changed\n");
        gizwitsProtocol.waitReportDatapointFlag.flagC2H5OH = 1;
        ret = 1;
    }
    if(last->valuefire_sensor != cur->valuefire_sensor)
    {
        GIZWITS_LOG("valuefire_sensor Changed\n");
        gizwitsProtocol.waitReportDatapointFlag.flagfire_sensor = 1;
        ret = 1;
    }
    if(last->valuetemp_alert != cur->valuetemp_alert)
    {
        GIZWITS_LOG("valuetemp_alert Changed\n");
        gizwitsProtocol.waitReportDatapointFlag.flagtemp_alert = 1;
        ret = 1;
    }
    if(last->valuehumi_alert != cur->valuehumi_alert)
    {
        GIZWITS_LOG("valuehumi_alert Changed\n");
        gizwitsProtocol.waitReportDatapointFlag.flaghumi_alert = 1;
        ret = 1;
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , uint8_t *outData,uint32_t *outDataLen)
{
    ...
    /*** Fill the bit ***/
    if(gizwitsProtocol.waitReportDatapointFlag.flagC2H5OH)
    {
        gizVarlenCompressValue(bitFieldOffset,C2H5OH_LEN,(uint8_t *)&allDatapointByteBuf[byteFieldOffset],dataPoints->valueC2H5OH);
        bitFieldOffset += C2H5OH_LEN;
    }
    if(gizwitsProtocol.waitReportDatapointFlag.flagfire_sensor)
    {
        gizVarlenCompressValue(bitFieldOffset,fire_sensor_LEN,(uint8_t *)&allDatapointByteBuf[byteFieldOffset],dataPoints->valuefire_sensor);
        bitFieldOffset += fire_sensor_LEN;
    }
    if(gizwitsProtocol.waitReportDatapointFlag.flagtemp_alert)
    {
        gizVarlenCompressValue(bitFieldOffset,temp_alert_LEN,(uint8_t *)&allDatapointByteBuf[byteFieldOffset],dataPoints->valuetemp_alert);
        bitFieldOffset += temp_alert_LEN;
    }
    if(gizwitsProtocol.waitReportDatapointFlag.flaghumi_alert)
    {
        gizVarlenCompressValue(bitFieldOffset,humi_alert_LEN,(uint8_t *)&allDatapointByteBuf[byteFieldOffset],dataPoints->valuehumi_alert);
        bitFieldOffset += humi_alert_LEN;
    }
